...
exports.getData = function(cb) {
  ...
  var req = http.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (ret.data.CasaCambioVentaValue && ret.data.CasaCambioCompraValue) {
        var ventaDecimal = parseFloat(ret.data.CasaCambioVentaValue.replace(/,/, '.')),
          compraDecimal = parseFloat(ret.data.CasaCambioCompraValue.replace(/,/, '.'));
